...
**Why:** The app supports English and Vietnamese. Any hardcoded string will not be translated.
...
// en.json: { "products": { "new": "New Product" } }
// vi.json: { "products": { "new": "San pham moi" } }
```
...
5. **Forgetting to add i18n keys to both `en.json` and `vi.json`** when adding new UI text.
