...
The i18n system uses react-i18next with JSON translation files. The template ships with English and Vietnamese.
...
### 4.2 Update Vietnamese Translations

**File:** `src/i18n/locales/vi.json`

Mirror the exact same key structure as `en.json`. Every key present in `en.json` must exist in `vi.json` or the TypeScript type system will error (the `en.json` type is used as the reference via `CustomTypeOptions`).
...
"ios": {
  "infoPlist": {
    "CFBundleLocalizations": ["en", "vi", "fr"]
  }
},
"plugins": [
  [
...
### 4.4 Remove Vietnamese Support

If your app does not need Vietnamese:

- [ ] Step 1: Delete `src/i18n/locales/vi.json`

- [ ] Step 2: Remove the Vietnamese resource from `src/i18n/config.ts`:
...
i18n.use(initReactI18next).init({
  resources: {
    en: { translation: en },
    // Remove: vi: { translation: vi },
  },
});
```

- [ ] Step 3: Update `app.json` to remove Vietnamese from `CFBundleLocalizations` and `supportedLocales`.
...
Mirror the same keys in `src/i18n/locales/vi.json` with Vietnamese translations.
...
- [ ] All new feature keys are present in both `en.json` and `vi.json` (or whichever locales you support)
...
- [ ] Language switching (if supported) works correctly for Vietnamese
