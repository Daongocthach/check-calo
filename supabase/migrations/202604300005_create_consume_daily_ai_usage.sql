DROP FUNCTION IF EXISTS public.consume_ai_usage(text, integer);
DROP FUNCTION IF EXISTS public.consume_ai_usage(integer, text);
DROP FUNCTION IF EXISTS public.consume_daily_ai_usage(text, integer);

CREATE FUNCTION public.consume_daily_ai_usage(
  p_purpose text,
  p_limit integer
)
RETURNS TABLE (
