...
- react-i18next (English + Vietnamese)
...
// src/i18n/locales/en.json  AND  src/i18n/locales/vi.json
```
...
The app supports English and Vietnamese. Keep user-facing strings in locale files and verify language switching still works after UI changes.
...
3. Ensure both `en.json` and `vi.json` are updated if UI text was added
