...
function run() {
  ...
  if (fs.existsSync(viPath)) {
    try {
      ...
      viKeys = countKeys(vi);
      pass(`vi.json parses OK (${viKeys} keys)`);
    } catch {
      fail('vi.json could not be parsed');
    }

    // 7. i18n key parity
    if (enKeys > 0 && viKeys > 0) {
      if (enKeys === viKeys) {
        pass(`i18n parity: ${enKeys} keys in both files`);
      } else {
        warn(
          `i18n mismatch: en has ${enKeys} keys, vi has ${viKeys} keys — run npm run i18n:check`
        );
      }
    }
  }
