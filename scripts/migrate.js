...
async function stepAppIdentity(rl) {
  ...
  if (shouldUpdateWelcome) {
    ...
    const viName = await ask(rl, 'App name in Vietnamese (or press Enter to skip)', '');
    if (viName && fileExists(viPath)) {
      const vi = readJSON(viPath);
      vi.home = vi.home || {};
      vi.home.welcome = `Chao mung ban den voi ${viName}`;
      vi.home.subtitle = `Ung dung ${viName} da san sang de su dung.`;
      writeJSON(viPath, vi);
      success('Vietnamese welcome text updated');
    }
  }
}
...
async function stepLanguages(rl) {
  ...
  const keepVietnamese = await askYesNo(rl, 'Keep Vietnamese (VI) translations?', true);

  if (!keepVietnamese) {
    const removeVietnamese = await askYesNo(
      rl,
      'Remove Vietnamese locale file and simplify i18n config?'
    );

    if (removeVietnamese) {
      // Remove vi.json
      const viPath = path.join(ROOT, 'src', 'i18n', 'locales', 'vi.json');
      if (fileExists(viPath)) {
        fs.unlinkSync(viPath);
        success('Removed vi.json');
      }
      ...
      if (fileExists(configPath)) {
        ...
        // Remove Vietnamese import line
        config = config.replace(/import\s+vi\s+from.*\n/g, '');
        // Remove vi: { translation: vi } from resources object
        config = config.replace(/,?\s*\bvi\b:\s*\{\s*translation:\s*\bvi\b\s*\}/g, '');
        // Remove 'vi' from array literals like fallbackLng: ['en', 'vi']
        config = config.replace(/,\s*['"]vi['"]/g, '');
        writeFile(configPath, config);
        success('i18n config simplified (VI removed)');
      }
      ...
      warn(
        'You may need to remove any unused locale-specific settings from app.json if not needed.'
      );
    }
  }
...
`;
    ...
    if (fileExists(viPath)) {
      const vi = readJSON(viPath);
      vi.tabs = vi.tabs || {};
      vi.tabs[fileName] = tabName; // placeholder — user should translate
      writeJSON(viPath, vi);
    }
  }
