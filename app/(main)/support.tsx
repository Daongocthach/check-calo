...
export default function SupportScreen() {
  ...
  const accountHolder = t('supportScreen.accountHolder');
  const accountNumber = t('supportScreen.accountNumber');
  const bankName = t('supportScreen.bankName');

  const transferClipboardText = useMemo(
    () =>
      [
        `${t('supportScreen.accountHolderLabel')}: ${accountHolder}`,
        `${t('supportScreen.accountNumberLabel')}: ${accountNumber}`,
        `${t('supportScreen.bankLabel')}: ${bankName}`,
      ].join('\n'),
    [accountHolder, accountNumber, bankName, t]
  );
  ...
  const handleCopyAccount = useCallback(async () => {
    ...
    try {
      await Clipboard.setStringAsync(transferClipboardText);
      setCopySuccessVisible(true);
    } finally {
      setIsCopying(false);
    }
  }, [isCopying, transferClipboardText]);
